...
def serialize(json_obj):
    ...
    len_to_bytes = len(json_str).to_bytes(LENGTH_FIELD_LENGTH, byteorder='little')
    return len_to_bytes + json_str.encode()
...
class Deserializer(object):
    ...
    def deserialize(self):
        json_str = self.buf[LENGTH_FIELD_LENGTH:LENGTH_FIELD_LENGTH + self.buflen].decode()
        self.buf = self.buf[LENGTH_FIELD_LENGTH + self.buflen:]
        self.buflen = -1
